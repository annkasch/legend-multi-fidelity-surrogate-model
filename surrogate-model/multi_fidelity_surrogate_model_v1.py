...
def AddNewSample(x_train_l, y_train_l, x_train_h, y_train_h, mf_model, labels, sample, version='v1'):
    ...
    for i in row_h:
        ...
        y_train_h=np.append(y_train_h,np.array([[data.iloc[i]['Ge77-Rate[nucleus/(kg yr)]']]]),axis=0)
    ...
    for i in row_l:
        ...
        y_train_l=np.append(y_train_l,np.array([[data.iloc[i]['Ge77-Rate[nucleus/(kg yr)]']]]),axis=0)
...
def DrawModelAcquisitionFunction(fig1, fig2, labels, version='v1'):
    ...
    with PdfPages(f'out/{version}/model-acquisition-evolution_{version}.pdf') as pdf:
        for index, label in enumerate(labels):
            ...
            curve0=ax0.lines[len(ax0.lines)-2]
