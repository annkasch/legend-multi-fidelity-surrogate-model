...
def get_neutron_info(filename):
    ...
    print(f"{tracksize} maximal steps per track")
...
#def main(filename, start=0, end=0, mode='LF', params=""): # jupyter notebook
    ...
    for file in filenames:
        ...
        if not params:
            ...
            #if (end>0):
            #    idx=int(file[file.find('LF-')+3:])
            #else:
            #   idx=int(file[file.find('LF-')+3:file.find('_',file.find('LF-'))])
            #params=f"{get_design_parameters_from_csv('../sampling/out/v2/LF-sim-samples_v2.csv',0)}".replace(' ', '')
            #tmp=get_design_parameters_from_csv('../sampling/out/v2/LF-sim-samples_v2.csv',idx)
            #tmp[len(tmp)-1]=get_volume_from_root(file)
            #params=f"{tmp}".replace(' ', '')
        ...
        try:
            os.remove(f"{file[:file.find('.root')]}.csv")
        except FileNotFoundError:
            pass
        writing_mode='w'
        print(f"Writing {file[:file.find('.root')]}.csv", flush=True)
        if params:
            f = open(f"{file[:file.find('.root')]}.csv", "w")#
            f.write(f"# [{mode}, {', '.join(params[1:-1].split(','))}] # mode design r d npanels phi theta L H z V"+"\n")
            f.close()
            writing_mode='a'
        startTime = datetime.now()
        df_out.to_csv(f"{file[:file.find('.root')]}.csv", mode=writing_mode)
