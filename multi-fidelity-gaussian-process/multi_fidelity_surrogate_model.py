...
def AddNewSample(x_train_l, y_train_l, x_train_h, y_train_h, mf_model, labels, sample, version='v1'):
...
def AddNewSamples(x_train_l, y_train_l, x_train_h, y_train_h, mf_model, labels, version='v1', sample=-1):
    sample_stop=GetNumberOfNewSamples(version)[0]
    if sample < sample_stop and sample >= 0:
        sample_stop=sample

    for i in range(sample_stop+1):
        x_train_l, y_train_l, x_train_h, y_train_h, mf_model = AddNewSample(x_train_l, y_train_l, x_train_h, y_train_h, mf_model, labels, i, version)
    return x_train_l, y_train_l, x_train_h, y_train_h, mf_model
...
def ActiveLearning(x_train_l, y_train_l, x_train_h, y_train_h, mf_model, xmin, xmax, xlow, xhigh, labels, factor, fig1, fig2, version, x_fixed, sample):
    ...
    if sample > 0:
        x_train_l, y_train_l, x_train_h, y_train_h, mf_model = AddNewSamples(x_train_l, y_train_l, x_train_h, y_train_h, mf_model, labels, version, sample-1)
    ...
    DrawMultiFideliyModel2D(mf_model,xmin, xmax,index1=0,index2=2,labels=labels,version=version,ml=0., x_fixed=x_fixed)
    DrawMultiFideliyModel3D(mf_model,xmin, xmax,index1=0,index2=2,index3=3,labels=labels,version=version,ml=1., x_fixed=x_fixed)
    ...
    #simulation_utils.PrintGeant4Macro(x_next_sample[0][0],x_next_sample[0][1],x_next_sample[0][2],x_next_sample[0][3],x_next_sample[0][4],sample,0.,version)
    #simulation_utils.PrintGeant4Macro(x_next_sample[0][0],x_next_sample[0][1],x_next_sample[0][2],x_next_sample[0][3],x_next_sample[0][4],sample,1.,version)
    #drawing_utils.DrawModeratorConfiguration(x_next_sample[0][0],x_next_sample[0][1],x_next_sample[0][2],x_next_sample[0][3],x_next_sample[0][4])
  
    fig2 = plt.subplot() #DrawAcquisitionFunction(fig2, us_acquisition, xlow, xhigh, labels, np.array(x_next_sample), version, x_fixed)
    return mf_model, fig1, fig2
...
def ActiveLearningModelOnly(x_train_l, y_train_l, x_train_h, y_train_h, mf_model, xmin, xmax, xlow, xhigh, labels, factor, fig1, fig2, version, x_fixed, sample):
    ...
    if sample > 0:
        ...
        y_new_data_h=np.array(y_train_h[len(y_train_h)-1])
        print(f"Adding new samples at {x_new_data} with Ge-77 Rate of {y_new_data_h} (HF) and {y_train_l[len(y_train_l)-1]} (LF)")
        drawing_utils.DrawModeratorConfiguration(x_new_data[0],x_new_data[1],x_new_data[2],x_new_data[3],x_new_data[4])
    ...
    with PdfPages(f'out/{version}/neutron-moderator-multi-fidelity-model_{version}.pdf') as pdf:
        for i in range(0,5):   
            ...
            if x_new_data.any() and y_new_data.any():
                plt.plot(x_new_data[i], y_new_data, color="orange", marker=".", markersize=10, linewidth=0)
            plt.xlabel(labels[i])
            plt.ylabel(r'Neutron Capture Probability')
...
def DrawMultiFideliyModel3D(mf_model,xmin, xmax, labels,index1=0,index2=3,index3=2,ml=1.0,version='v1', x_fixed=[160., 5., 40., 45., 100.]):
    SPLIT=25
    i=index1
    j=index2
    m=index3
...
def DrawMultiFideliyModel2D(mf_model,xmin, xmax, labels,index1=0,index2=3,ml=1.,version='v1', x_fixed=[160., 5., 40., 45., 100.]):
    SPLIT=100
    i=index1
    j=index2
...
def DrawUpdatedMultiFideliyModel(fig, x_train_l,y_train_l,x_train_h,y_train_h,mf_model,xmin, xmax, labels, factor=1., x_new_data=np.array([]),y_new_data=np.array([]), version='v1', x_fixed=[200., 10., 40., 45., 30.]):
    ...
    with PdfPages(f'out/{version}/updated-neutron-moderator-multi-fidelity-model_{version}.pdf') as pdf:
        for i in range(0,5):   
            ...
            if x_new_data.any() and y_new_data.any():
                ax2.plot(x_new_data[i], y_new_data, color="orange", marker=".", markersize=10, linewidth=0)
            ax2.set_xlabel(labels[i])
            ax2.set_ylabel(r'Neutron Capture Probability')
...
def DrawAcquisitionFunction(fig, us_acquisition, xlow, xhigh, labels, x_next=np.array([]), version='v1', x_fixed=[160,5,40,45,100]):
    SPLIT = 50
    df= pd.DataFrame()
    with PdfPages(f'out/{version}/acquisition-function_{version}.pdf') as pdf:
        for i in range(0,len(xlow)):
            ax2 = fig[i].gca()
            ax2.set_title(f'Projected acquisition function - {labels[i]}');
            x_plot=[x_fixed[:] for l in range(0,SPLIT)]
...
def ReadInAcquisitionFunction(filename, labels):
